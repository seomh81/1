...
def start_selltrade(target_item, buy_amt):#(target_item, minute_interval, minute_check, buy_amt):
    try:
        ...
        while True:
            ...
            time.sleep(0.3) # 타임슬립을 넣어주지 않으면 조회수가 너무 많아 팅김. 0.1 이상일 것.
    ...
    try:
        ...
        while True:
            ...
            for item_list_for in item_list:
                ...
                val_b4 = h_b4 - l_b4
                val_b5 = h_b5 - l_b5
                val_now = val_b1 - val_b2 - val_b3 - val_b4
                ...
                df_bb1 = upbit.get_bb(item_list_for['market'], '1', '200', 1) #3분봉으로 변경

                val_bb1 = df_bb1[0]['BBL']
                bb1_now = c_now - val_bb1
                bb1_now_per = bb1_now / val_bb1 * 100
                print(str(format(bb1_now_per, ".2f")) + " %", "  ★   가격차 " + str(format(val_now / c_now * 100, ".2f")) + "%", item_list_for['market'])#, "■  거래량 " + str(format(mv_check, ".1f")) + "%")
                # 100시간 전 거래량
                ...
                # 10분 전체 거래량 합보다 현 거래량이 클 경우 매수 / 체결량 제거
                if l_b1 < val_bb1 \
                        and val_now > 0 :
                        #and val_b1 != 0 \
                        #and val_b2 != 0 \
                    ...
                    upbit.buycoin_tg(item_list_for['market'],buy_amt, h_b1)
