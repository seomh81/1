    ...
    try:
        ...
        while True:
            ...
            for target_item in target_items:
                ...
                # -------------------------------------------------------------
                indicators = upbit.get_indicator_sel(target_item['market'], '1', 100, 5,
                                                     ['CANDLE'])
                ...
                gogogo = (Decimal(candle[0]['trade_price']) - Decimal(candle[0]['opening_price'])) / Decimal(candle[0]['opening_price']) * 100
                logging.info(gogogo)
                if gogogo >= 1.1:
...
if __name__ == '__main__':
    ...
    try:
        ...
        #buy_amtp = 90 #몇 %씩 매수할지?
        upbit.set_loglevel(log_level)

        logging.info("*********************************************************")
        logging.info("1. 로그레벨 : " + str(log_level))
        logging.info("2. 매수금액 % : 총액의 " + str(buy_amt) + "%")
        logging.info("*********************************************************")
